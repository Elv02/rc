...
class Raycaster {
     ...
     */
    getShortestLineFromPointToPoints(origin: Phaser.Geom.Point, points: Phaser.Geom.Point[]): Phaser.Geom.Line | undefined {
        ...
        if (points.length > 0) {
            ...
            return new Phaser.Geom.Line(origin.x, origin.y, closestIntersection?.x, closestIntersection?.y);
        } else {
            return undefined;
        }
    }
     ...
     */
    getIntersection(ray: Phaser.Geom.Line): Phaser.Geom.Point | undefined {
        let closestIntersection: Phaser.Geom.Point | undefined = undefined;
        ...
        for (let y: number = 0; y < this.level.length; y++) {
            for (let x: number = 0; x < this.level[y].length; x++) {
                if (this.level[y][x] === 1) {
                    ...
                    const shortLine = this.getShortestLineFromPointToPoints(ray.getPoint(0), points);
                    const shortLen = this.getLineLength(shortLine);
                    if (shortLen < closestDistance) {
                        closestDistance = shortLen;
                        closestIntersection = shortLine?.getPoint(1);
                    }
                }
            }
        }
